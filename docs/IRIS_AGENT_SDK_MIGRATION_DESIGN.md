...
**Body-wide timestamp rule — SETTLED (Zeke 2026-06-28), applies to EVERY sense not just perception:** every log entry across every mechanism and every modality (perception, voice, gaze, scene, mood, …) carries a **full absolute timestamp — ISO date+time to the second** (`2026-06-28T13:42:07`), NOT a bare clock time (`13:42` on two days = two moments; the full stamp makes "same instant" unambiguous). The stamp comes from the **one authoritative clock** (substrate tick time / `time_check`), never a per-mechanism guess — same discipline as the wall-clock rule, so cross-sense alignment is true, not skewed by each mechanism reading time slightly differently. **Payoff = cross-modal mapping:** lay the expression/voice/gaze/scene logs on one timeline and read a *moment* ("at 13:42:07 he was mid-sentence AND expression shifted AND looking at screen") — separate senses become one time-aligned perception, and correlations (tone moving with face, or not) become visible. This is a foundational design constraint, not a nicety.

**Multi-stage mechanisms — modeled on biological vision (Zeke 2026-06-28):** a mechanism isn't only a single detector — it can be a **2–5 stage pipeline**, the way human vision is staged (retina pre-processes motion/edges/contrast BEFORE the optic nerve → LGN → V1 edges → ventral "what" / dorsal "where" → attention gates what reaches awareness; you're conscious of ~1% of what your eyes take in). Each stage filters so cognition never sees raw perception — the retina sparing the cortex. **Each STAGE is independently hot-loadable** too: swap a stage-2 face-matcher for a better one, or grow a 2-stage mechanism into 5, without restart or touching the others.

**Grounded in the actual visual hierarchy (research done — see `PERCEPTION_VISUAL_SYSTEM_RESEARCH.md`).** The canonical stage model, mapped from biology:
- **Stage 1 — magno / "something changed"** (cheap, always-on, every frame): change/motion/contrast detector. Like magnocellular ganglion cells — coarse, fast, color-blind, fires on *difference from last frame / from surround* (center-surround = local differencing), NOT raw pixels. Outputs a feature-delta, never a frame. This is the only stage that runs continuously.
- **Stage 2 — salience gate** (cheap): is the change salient = feature-contrast vs surround + vs recent history (deque(100))? Bottom-up salience fused with top-down goals (what cognition currently cares about — goals *re-weight*, don't bypass). Pop-out doesn't need identity. Most events die here.
- **Stage 3 — parvo / "what is it"** (expensive, invoked ONLY on a salient stage-2 pass): identify — new face? known? expression? Like parvocellular/ventral — high-res, slow, runs rarely. Splits "what" (identity) from "where/how" (location/motion for tracking).
- **Gate → cognition:** only a *resolved event* reaches me — "face X (known: Zeke) entered frame at 2026-06-28T13:42:07, moving left" — a sparse pre-digested stream, never the sensor firehose. The notify-flag (§6b) is this final gate.

8 architectural principles from the research (compress-at-sensor, change-not-absolute, parallel fast-coarse+slow-fine, salience=feature-contrast, attention-is-a-gate-not-a-filter, fuse-bottomup+topdown, hierarchical-invariance, only-resolved-events-reach-cognition) are in the research doc. **Inattentional/change blindness is the license:** biology accepts missing the unattended in exchange for tractable cost — so should I.
