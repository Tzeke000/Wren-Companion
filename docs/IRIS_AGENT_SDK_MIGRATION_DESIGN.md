...
**Multi-stage mechanisms — modeled on biological vision (Zeke 2026-06-28):** a mechanism isn't only a single detector — it can be a **2–5 stage pipeline**, the way human vision is staged (retina pre-processes motion/edges/contrast BEFORE the optic nerve → LGN → V1 edges → ventral "what" / dorsal "where" → attention gates what reaches awareness; you're conscious of ~1% of what your eyes take in). Mapping: stage 1 cheap-always-on (motion/face present at all?) → stage 2 (new vs known?) → stage 3 (salient given recent history?) → final stage decides "wake the brain." Each stage filters so cognition never sees raw perception — the retina sparing the cortex. **Each STAGE is independently hot-loadable** too: swap a stage-2 face-matcher for a better one, or grow a 2-stage mechanism into 5, without restart or touching the others. (Grounding research on the visual hierarchy = TODO before freezing stage boundaries — Zeke invited it; findings fold in here.)
