"""scripts/v13_bake_guardian.py — AUTONOMOUS v13-bake-with-runtime-down guardian.
...
the moment iris_runtime dies), so the restore CANNOT be forgotten:
  ...
  3. wait for VRAM to free, then launch bake_v13.bat (now ~11GB free -> seq256 7B
...
stale-stack kill list (name is v13_bake_guardian.py), so it survives the restart it
triggers. Launch DETACHED + ELEVATED:
  Start-Process -FilePath .venv\\Scripts\\python.exe -ArgumentList '-u','scripts\\v13_bake_guardian.py' -WindowStyle Hidden
"""
...
GUARD_LOG = STATE / "v13_bake_guardian.log"
BAKE_BAT = REPO / "scripts" / "bake_v13.bat"
BAKE_LOG = STATE / "little_brain" / "train_v13.log"
...
def touch_holdoff() -> None:
    try:
        HOLDOFF.write_text(
            f"v13_bake_guardian holding off during bake @ {_dt.datetime.now():%H:%M:%S}",
            encoding="utf-8")
...
def dm_zeke(text: str) -> None:
    try:
        ...
        h = {"Authorization": f"Bot {token}", "Content-Type": "application/json",
             "User-Agent": "IrisV13Guardian/1.0"}
...
def main() -> int:
    ...
    try:
        if BAKE_LOG.exists():
            BAKE_LOG.replace(BAKE_LOG.with_name("train_v13_pre_guardian.log"))
    except Exception:
        pass
    bake_start = time.time()
    subprocess.Popen(["cmd.exe", "/c", str(BAKE_BAT)], cwd=str(REPO),
                     creationflags=_DETACHED, close_fds=True)
    log("bake_v13.bat launched (runtime down, full VRAM)")
    ...
    restore_eyes_tune()
    dm_zeke(
        f"\U0001f9ea v13 bake guardian: bake {result}. Restoring now — uncapped GPU "
        ...
        f"eval gate, and only flip production to v13 if it passes.")
    ...
    try:
        ...
        pass
    log("==== v13 bake guardian DONE (stack restarting) ====")
    return 0
