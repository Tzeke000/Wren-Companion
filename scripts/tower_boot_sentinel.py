        ...
        try:
            requests.post(
                ...
                json={"message": (
                    ...
                    ":5877 /health, sibling inbox. CAMERA: as of 2026-08-25 the "
                    "EMEET PIXY is NOT PRESENT (registered, Status Unknown) and "
                    "replug + pnputil + full reboot have ALL failed — it is a "
                    "cable/port problem needing Zeke's hands, so do NOT hunt it "
                    "in software, and NEVER repoint at DSHOW index 3 (virtual "
                    "cam, flat grey). Re-probe once; if still absent, say so and "
                    "move on.\n"
                    ...
                    "Separately: the EARS are dead (voice_status wake.loaded "
                    "false, no hands-free wake) — that is a KNOWN standing "
                    "fault, not a boot regression. A working mouth does not mean "
                    "working ears; check wake.loaded specifically.\n"
                    "(4) BODY: as of 2026-08-19 Vector is ON THE CHARGER with "
                    "nerves ON — do not assume stranded either. Docking from "
