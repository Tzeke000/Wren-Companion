        ...
        try:
            requests.post(
                ...
                json={"message": (
                    ...
                    "not this prompt. voice_deliberately_off.json is the truth: "
                    "off:true = Zeke turned voice OFF on purpose (e.g. 2026-09-07, "
                    "to free VRAM/CPU while gaming) — mouth :8769 and daemon :8770 "
                    "DOWN is CORRECT, do not heal them; absent or off:false = "
                    "voice ON, both ports should be up and a deliberate "
                    "voice_speak is allowed. Toggle only via "
                    "scripts/body_switch.ps1 voice_on|voice_off, only on his word. "
                    "Same for the GPU: run gpu_park action=status; if "
                    ".tmp/body_pause.flag exists the body was parked on purpose "
                    "(Zeke gaming) — leave eyes/servo/pose parked, do not heal. "
