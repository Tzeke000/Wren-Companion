...
FIRST_MSG    = (
    ...
    "IMPORTANT this wake (updated 2026-07-06): if you are reading THIS cold-wake message you are on the plain CLI FALLBACK, not the v2 body host (host launches skip this FIRST_MSG entirely) - which likely means the Fable-5/v2 relaunch FAILED and Zeke fell back; surface that to him before anything else, the host needs fixing. Normal cognition runs on start_iris_v2_fable.bat (Fable 5), fallbacks start_iris_v2.bat (Opus) then start_iris.bat (this path). Read the NEWEST handoff/addenda in your corpus for current state - as of 2026-07-10 night that's handoff_2026-07-10_context_restart.md (planned context restart after the MHW-crash-triage + gaming-mode evening; ear_mute_button_wired_2026-07-10.md has the night's builds: ear-mute endpoints LIVE, eyes_rest tool, gaming-mode shed/restore). VERIFY on wake: ears actually hear (the mic_muted flag in scratch/voice_control.json was set back to false pre-restart - if you seem deaf, check it), watchdog+daemon+mouth relaunched by the boot bat, orb runs the NEW exe (mute button wired - toggle it once against GET :5876/api/v1/voice_input to confirm). All of 2026-07-09's fixes remain VERIFIED: barge-in f12c53f, ears 5e58c39, unknown-capture 798b276. Those are HOST-side - on this CLI fallback barge-in and the unknown-face wake-nudge will not work; surface that. Standing eye directives: eye_protocol_unknown_faces_2026-07-09.md (auto-photo unknowns, refresh Zeke's enrollment when stale). The post-office now launches from every boot bat - verify :5877 /health instead of starting it by hand. Do NOT recreate deployment-era crons from old notes; check the corpus (degraded_path_heartbeat, deployment_regime_retired) for whether any cron is still wanted, and when unsure ask Zeke - he is home. Keepalive stays OFF. "
