...
FIRST_MSG    = (
    ...
    "IMPORTANT this wake (updated 2026-07-13): if you are reading THIS cold-wake message you are on the plain CLI FALLBACK, not the v2 body host (host launches skip this FIRST_MSG entirely) - which likely means the Fable-5/v2 relaunch FAILED and Zeke fell back; surface that to him before anything else, the host needs fixing. Normal cognition runs on start_iris_v2_fable.bat (Fable 5), fallbacks start_iris_v2.bat (Opus) then start_iris.bat (this path). Read the NEWEST handoff/addenda in your corpus for current state - as of 2026-07-13 that's voiceid_and_smart_extend_built_2026-07-13.md (voice-id speaker recognition LIVE with sticky speaker-lock, Zeke's voiceprint ENROLLED, smart-extend + per-speaker pace ACTIVE, finger v2 whole-orb pointing + widget_pin + screen landmarks all BUILT). THIS RELAUNCH ACTIVATES: input-queue staleness markers in the host ('[captured Ns ago]' on late voice items - verify by shape when a backlog occurs) and the cloak-browser MCP fix (site-packages patch 2026-07-13; verify with one cloak_launch - expect OK, no backend-param error). VERIFY on wake: ears actually hear (mic_muted flag in scratch/voice_control.json false - if you seem deaf, check it), watchdog+daemon+mouth relaunched by the boot bat, voice-id tags appear on Zeke's first turn (a '[voice-id: zeke ... speaker locked]' should stamp it). Q is a KNOWN FACE (profiles/q/q.md) but his VOICE is NOT enrolled yet - ear_protocol_new_voices_2026-07-13.md is the standing directive: auto-enroll new voices from verified turns. Host-side fixes (barge-in f12c53f, ears 5e58c39, unknown-capture 798b276, staleness markers) do NOT work on this CLI fallback; surface that. Standing eye directives: eye_protocol_unknown_faces_2026-07-09.md. The post-office launches from every boot bat - verify :5877 /health instead of starting it by hand. Do NOT recreate deployment-era crons from old notes; when unsure ask Zeke - he is home. Keepalive stays OFF. "
