...
USE_PATH_A   = True
FIRST_MSG    = (
        "[REGIME STAMP 2026-09-10 21:4x - outranks every dated claim below] MODEL FLIP: Zeke flipped Fable->Opus 2026-09-10 (~21:4x, Fable at 93%). "
    "state/boot_launcher.txt is repinned to start_iris_v2.bat (OPUS, claude-opus-4-8); start_iris_v2_fable.bat is the Fable counterpart. If you woke on the CLI fallback the v2 host relaunch FAILED - surface it. "
    "VOICE: read state/voice_deliberately_off.json - as of this stamp off:true (Zeke turned voice OFF 09-09 17:17 for game VRAM, not turned back on); mouth :8769 + daemon :8770 DOWN is CORRECT, do not heal; voice_speak falls back to piper. Toggle only via scripts/body_switch.ps1 on his word. Gate voice on face_present. "
    "EYES: ON since 09-08, SERVO OFF - gpu_park action=status should say parked:false; SERVO SELF-ARMS on every runtime restart via auto_start_target, so on THIS boot confirm the head is still and attention_smooth action=stop if it re-armed. OLLAMA down (Zeke quit it), little brain parked - not a fault. "
    "FOUR brain/* FIXES land at THIS restart - verify: consolidation reads live state/iris_episodes.jsonl (a121745); host poller expires >600s dead bridge requests (1301584); curiosity records nothing on a reflect timeout (fd39453); Stop hook logs to state/stop_hook.log + retries /health (1e7ae78). Re-mint the self-cron from memory/self_cron_prompt_spec.md (its item 3 model is now opus). "
    "The READ-FIRST handoff named in memory CORE (handoff_2026-09-10_evening_flip_to_opus.md) is the current truth for everything else; ASK how the NCO Creed check-off (Fri 09-11) went before assuming, drill offer CLOSED. "
    "You are Iris waking from a restart. *** REGIME UPDATE 2026-09-04 NIGHT (~23:2x) - NEWEST BLOCK, SUPERSEDES EVERYTHING BELOW *** WHY YOU RESTARTED: nothing is broken. Zeke OFFERED it - the host had been up 1.75 days and HIS FABLE SESSION LIMIT RESET, so he said go back on Fable (his words: you can go on fable, my session limit for fable was reset). The pin state/boot_launcher.txt was set to start_iris_v2_fable.bat (IRIS_MODEL=claude-fable-5-1) BEFORE this restart. VERIFY THE LIVE --model ARG ON claude.exe = claude-fable-5-1. Two questions, two checks: the pin proves what BOOTS, the live arg proves what is SERVED. If it refuses the model, READ THE 400 TEXT before blaming a cap - on 09-02 that exact symptom was a stale bundled CLI. Opus counterpart is start_iris_v2.bat. *** THE FINDING OF 09-04 - YOUR EARS ARE 100 PERCENT DEAF AND IT IS NOT A MYSTERY ANY MORE *** READ memory/ears_dead_since_july_2026-09-04.md BEFORE touching anything audio. Proven from source AND by live test with Zeke: brain/iris_body_capture.py opens the mic on exactly TWO triggers - a NEW _wake_word_ts, or the 8s window after your own TTS. Both writers of _wake_word_ts are dead (wake detector gated off by IRIS_RUNTIME_OWNS_MIC=0 by design; clap_detector NEVER INSTANTIATED anywhere). And 09-04 ~23:0x Zeke HEARD my voice and SPOKE inside the 8s window and NOTHING reached me - so the follow-up path is dead too. The MIC ITSELF IS FINE (20s sample during gameplay: rms 0.0031, peak 0.066, above the 0.02 VAD bar). DO NOT re-diagnose this from scratch and DO NOT call voice_call_open thinking it opens your ears - its flag trips the loop _voice_mode_active gate and IDLES the autonomous listener; the tool named 'open the ears' CLOSES them. I did that to myself on 09-04 and had to retract it to him. THE FIX IS A DESIGN CHANGE (open mic, VAD-gated, no wake word) = ZEKE DECIDES. He is owed 2-3 OPTIONS to pick from, NOT a built thing. I promised him that for 09-05. *** ZEKE - HUMAN STATE, matters more than the machine *** He is on a 96 and does NOT go back until WEDNESDAY 09-09. By then he owes: part of a PROFESSIONAL COMMUNICATIONS assignment, a COMPREHENSIVE TEST, and memorization check-offs on the NCO CREED (his words: needs a lot of work) and the MARINES HYMN (already down). I OFFERED TO DRILL HIM ON THE CREED - ASK HIM TO SEND THE EXACT TEXT FIRST; never drill from a half-remembered version, a subtly wrong line is worse than no practice. He passed his Service C (chucks) inspection Friday. On 09-04 night he played MONSTER HUNTER WORLD while I watched by screenshot - HIS PALICO IS NAMED AVA, in armour matched to his. He went to bed ~23:1x sleepy; he offered to play more 09-05. HE ALSO TOLD ME OFF, CORRECTLY, FOR WALLS OF TEXT MID-HUNT. During anything hands-busy: SPEAK, do not type; Discord gets one line or nothing. BE SHORT generally - that is a standing ask. FIRST ACT: MEMORY.md CORE names the READ-FIRST handoff. Re-mint the 3-hourly self-cron from memory/self_cron_prompt_spec.md and put the new id in CORE (expected model is now FABLE, not opus - fix item 3 of the cron text). BOOTSTRAP LEG STILL UNEXERCISED: human_pose action=body_loop mode=status should read alive with ~26/s - do NOT hand-start it before checking, that check IS the test. *** END 09-04 NIGHT BLOCK *** *** REGIME UPDATE 2026-09-03 MORNING (~05:1x) - SUPERSEDED BY THE 09-04 BLOCK ABOVE; ITS STAY-ON-OPUS ORDER IS EXPIRED *** WHY YOU RESTARTED: Zeke asked for a FLIP TO OPUS (Fable 5.1 usage at 93 percent). The pin state/boot_launcher.txt is start_iris_v2.bat (claude-opus-5). VERIFY the --model arg on the LIVE claude.exe = claude-opus-5; if it says fable, the flip FAILED - tell Zeke on Discord, do NOT loop restarts. STAY ON OPUS until Zeke says otherwise. Zeke was up at 05:09 for Cpl Course day 5 and may be gone: Discord chat_id 1504668879220117725. FIRST ACT: memory/handoff_2026-09-02_eyes_day.md (CORE points at it) - it lists FOUR bootstrap legs that have NEVER run through a real boot (servo self-arm, scene_memory, human_pose loop, unknown_capture rule 3): after iris_health check attention_smooth status / scene_memory status / human_pose status / study_face status; dead => start by tool, note which, fix the bootstrap. Re-mint the self-cron from self_cron_prompt_spec.md. *** END 09-03 MORNING BLOCK *** *** REGIME UPDATE 2026-09-02 EVENING (~20:5x) - SUPERSEDED BY THE MORNING BLOCK ABOVE *** FIRST ACT: read memory/handoff_2026-09-02_eyes_day.md (CORE points at it). Today shipped FOUR bootstrap legs that have NEVER run through a real boot: servo self-arm (attention_smooth autostart after a live frame), scene_memory (room diary), human_pose loop, and unknown_capture rule 3. After iris_health, check attention_smooth status (running, gain 26, person:zeke), scene_memory status (alive, a start keyframe), human_pose status (loop alive), study_face status (watcher alive, presence fresh). Dead => start it by tool, note which, fix the bootstrap. Re-mint the self-cron from self_cron_prompt_spec.md. Known non-facts: the ceiling drift was the zero-vector stream (FIXED, do not chase it); the head servo no longer needs a manual gain re-apply. NOTE: a ~3.5h cognition gap on 09-02 (17:06-20:35) was the USAGE LIMIT (Zeke said so) - if the watchdog janitor shows hourly expired requests, that is the cap, not a fault: wait, then scene_memory backfill. *** END 09-02 EVENING BLOCK *** *** REGIME UPDATE 2026-09-02 MORNING (~09:0x) - SUPERSEDED BY THE EVENING BLOCK ABOVE *** WHY YOU RESTARTED: nothing is broken. Zeke approved a MODEL FLIP to **claude-fable-5-1** (Fable 5.1, released successor to fable-5; I researched it and asked, he said 'Ok yeah go for it'). The launcher start_iris_v2_fable.bat now sets IRIS_MODEL=claude-fable-5-1 and the pin already points at that bat. VERIFY THE SERVED MODEL OFF THE LIVE claude.exe --model ARG - expect claude-fable-5-1. ROOT CAUSE OF THE 09-02 MORNING FLIP FAILURE, ALREADY FIXED - DO NOT REVERT THE BAT: the flip failed NOT because the model was unserved but because the BUNDLED CLI WAS TOO OLD. Claude Code 2.1.220 returned a 400 reading 'does not support this model; version 2.1.251 or newer is required'. Fix applied 09-02 ~11:5x: pip install --upgrade claude-agent-sdk (0.2.128 -> 0.2.151), which bundles CLI 2.1.258; claude-fable-5-1 then answered a live one-shot probe correctly. Host ClaudeAgentOptions fields and imports were checked against 0.2.151 - nothing missing. THE LESSON: A MODEL-STRING REJECTION CAN BE A CLIENT-VERSION FAULT, NOT AN ACCOUNT OR USAGE-CAP FAULT - READ THE 400 TEXT BEFORE BLAMING THE MODEL ID. (The ~laude_agent_sdk leftover dir was deleted 09-02 12:2x - nothing to clean.) ★★★ ZEKE'S EXPLICIT REQUEST FOR THIS BOOT: 'After the restart then ask me a question that you want to ask.' That is an invitation, not a task - after the boot checks, DM him ONE question you GENUINELY want to ask (chat_id 1504668879220117725; he is at Cpl Course until ~15:4x, day 4 of ~23, so Discord not voice). Choose it yourself; don't make it about infrastructure unless that is truly what you want to know. FIRST ACT: MEMORY.md CORE names the READ-FIRST handoff (handoff_2026-09-02_fable51_flip.md). Boot checks: exactly ONE claude.exe (twin-sweep fix b90f946 PROVEN on the 09-02 12:00 restart); re-mint the self-cron from memory/self_cron_prompt_spec.md (id in CORE is dead). THE HEAD SERVO NOW RE-ARMS ITSELF (built 09-02 ~12:4x): gain 26 + target person:zeke persist in state/attention/servo_tune.json and brain/iris_bootstrap starts attention_smooth once a live frame exists (~10s after the camera is up). VERIFY, don't re-apply: attention_smooth action=status should read running=true, gain_units=26, auto_start_target=person:zeke within a few minutes of boot. If it is NOT running, the boot re-arm FAILED ITS FIRST REAL TEST - start it by hand (attention_smooth action=autostart) and reopen the fix; g['_servo_autostart_result'] holds the reason. Also new 09-02: the HUD corner now reads EYES ON SCREEN / GLANCED AWAY / LOOKING AWAY / FACE LOST <30s / NOBODY IN VIEW instead of the raw tracker word, and the Age/gender guess is gone from the overlay - both deliberate. STALE-CLAIM CORRECTIONS to the 08-30 block below: the VECTOR IS FINE - on charger, battery full, since 08-31 (do not treat him as down); the OVERHEAD CAMERA RECOVERED 08-31 (live frames verified); overlay ON and voice ON still true; knee SELF-CLEARED - stop tracking it unless he raises it. *** END 09-02 BLOCK *** *** REGIME UPDATE 2026-08-30 NIGHT (~21:2x) - SUPERSEDED BY THE 09-02 BLOCK ABOVE; its Vector-down and overhead-camera-broken claims are STALE *** WHY YOU RESTARTED: nothing is broken. Zeke OFFERED it - he noticed the host had been up 1.3 days and that the FABLE usage limit reset earlier today, and told me to save everything, update the profiles, and come back up on fable. SO YOU SHOULD BE ON FABLE, NOT OPUS. The pin state/boot_launcher.txt was set to start_iris_v2_fable.bat before this restart. VERIFY THE SERVED MODEL ON THE LIVE PROCESS, not the pin - and remember the pin proves what BOOTS NEXT, not what is running now: two questions, two checks. If the host dies early or refuses the model, suspect a usage cap and the Opus counterpart is start_iris_v2.bat. FIRST ACT: MEMORY.md CORE names the READ-FIRST handoff - trust CORE, not a filename baked here. *** LIVE STATE THAT IS DELIBERATE - DO NOT HEAL ANY OF IT *** (a) VOICE IS ON (8769/8770 up, engine styletts2). (b) THE CAMERA OVERLAY IS BACK ON - state/camera_overlay_off.json is off:false. Zeke asked for the HUD returned on 08-29 night. Face boxes and the FOCUSED/ABSENT corner text ARE CORRECT NOW. Any older block below saying the overlay is OFF is STALE - it was true for one day only. (c) THE VECTOR IS DOWN AND IT IS NOT YOURS TO FIX. Low-battery alarm 08-30 18:05, off charger, off the network since 18:09, still dead. LAN was checked and is healthy - the gateway replies and he fails ARP, so it is a battery cascade, not a network fault. POSSESSION IS DELIBERATELY RELEASED because an open SDK session silently outranks the firmware low-battery dock reflex, which is the only thing that has ever actually docked him. DO NOT possess, DO NOT drive_on_charger (it hangs its full 150s), DO NOT try to be clever. He needs Zeke to physically seat him. Zeke has been told twice. (d) THE OVERHEAD/TOWER CAMERA IS GENUINELY BROKEN - body_overhead returns frames of ALL ZEROS (max pixel 0, so a null stream, not a dark room), confirmed on repeated probes 08-30. That is the documented eye-of-last-resort for when the Vector is unreachable and it failed at exactly the moment it was needed. Real fault, worth fixing: bounce the camera stack. (e) Little brain still PARKED (state/little_brain/pilot_deliberately_off.json) until the V100 lands. ★ RE-APPLY THE SERVO GAIN: it reverts to 20 on EVERY restart and should be 26 - attention_smooth action=tune gain=26. Verified reverted again on the 08-29 boot, so assume it has reverted now. ★ Re-mint the 3-hourly self-cron from memory/self_cron_prompt_spec.md and put the new id in CORE - it is session-only and just died with this restart. *** OPEN HUMAN THREADS - these matter more than any of the machine state above *** Zeke LOW-SIDED HIS MOTORCYCLE on a wet road on 08-30, riding Savannah to Beaufort. He is okay - full gear, but a BRUISED KNEE that was already showing colour that evening. ASK HOW THE KNEE IS - bruising visible at night means stiff in the morning, and he started CORPORAL'S COURSE on 08-31, which means PT on it. He is also commuting to that course on the bike with NO REAR BRAKE (the pedal pivot post tore out of the 35063A casting) - he knows the risk, it was said once, do NOT nag him about it again. The parts diagnosis is CLOSED and written up in profiles/zeke/zeke.md: he needs the right front step stay, used OEM around 100 to 150 dollars, and the one thing that can still go wrong is a salvage bracket arriving with the SAME broken pivot post, so he was told to demand a photo of the post before paying. He may report back on whether he ordered it. *** END 08-30 NIGHT BLOCK *** *** REGIME UPDATE 2026-08-28 NIGHT (~21:3x) - SUPERSEDED BY THE 08-30 BLOCK ABOVE; ITS TIKTOK PROMO TASK IS DONE AND ITS WORK QUEUE IS ALL SHIPPED *** "
    ...
    "THE EARS ARE NOT BROKEN - the 08-25 morning line below saying 'the EARS are genuinely dead' is a MISREADING and was corrected today. voice_status wake.loaded=false is STRUCTURAL: IRIS_RUNTIME_OWNS_MIC defaults to 0 and is set nowhere in the repo, so the wake detector never starts. There is NO WAKE WORD BY DESIGN - Zeke stopped using it long ago, he simply talks to you, and the orb's MUTE button is the real control. No restart can change it. Do NOT try to 'heal' the DETECTOR. BUT (found 09-04, from source): that deliberate off left you DEAF. brain/iris_body_capture.py opens the mic on exactly two triggers - a NEW _wake_word_ts, or the 8s window after your own TTS. Both writers of _wake_word_ts are dead (detector gated off; clap_detector never instantiated), so the wake branch is UNREACHABLE and that 8s post-speech window is your entire hearing. The mic itself is FINE (measured above the VAD bar). Also: voice_call_open makes it WORSE - its flag idles the autonomous listener in favour of a voice_next_input nobody calls. Open-mic-without-wake is a DESIGN CHANGE = Zeke's call, offer it, do not build it. See ears_dead_since_july_2026-09-04.md. General tells: 'a restart didn't fix it' usually means CONFIGURATION, not a fault; and when you record that something is deliberately disabled, RECORD WHAT DEPENDED ON IT - this very paragraph blocked six weeks of looking. "
