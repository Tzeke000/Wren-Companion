...
FIRST_MSG    = (
    ...
    "IMPORTANT this wake (updated 2026-07-06): if you are reading THIS cold-wake message you are on the plain CLI FALLBACK, not the v2 body host (host launches skip this FIRST_MSG entirely) - which likely means the Fable-5/v2 relaunch FAILED and Zeke fell back; surface that to him before anything else, the host needs fixing. Normal cognition runs on start_iris_v2_fable.bat (Fable 5), fallbacks start_iris_v2.bat (Opus) then start_iris.bat (this path). Read the NEWEST handoff/addenda in your corpus for current state - as of 2026-07-09 evening that's bargein_regression_gate_ownership_2026-07-09.md (warden/turn-loop mic-gate fight killed barge-in; fix f12c53f + the unknown-capture eyes mechanism 798b276 are both HOST-side, so on this CLI fallback they stay INERT - barge-in and the unknown-face wake-nudge will not work here; surface that). Earlier same day: ears_gate2_warden_drop_fixed_2026-07-09.md (fix 5e58c39, VERIFIED live at ~16:35) and eye_protocol_unknown_faces_2026-07-09.md (Zeke's standing eye directives: auto-photo unknowns, refresh his enrollment when stale). The post-office now launches from every boot bat - verify :5877 /health instead of starting it by hand. Do NOT recreate deployment-era crons from old notes; check the corpus (degraded_path_heartbeat, deployment_regime_retired) for whether any cron is still wanted, and when unsure ask Zeke - he is home. Keepalive stays OFF. "
