...
USE_PATH_A   = True
FIRST_MSG    = (
    "[REGIME STAMP 2026-09-07 16:0x - outranks every dated claim below] VOICE IS ON (StyleTTS2 mouth :8769 + daemon :8770 up; "
    "any line below saying voice is deliberately OFF or 8769/8770 DOWN is CORRECT is STALE). Ears may be silent because Zeke MUTED them "
    "with the orb button (his control, not a fault). Camera PRESENT. The GPU body may be PARKED for his gaming: run "
    "gpu_park action=status FIRST - parked means paused engines, no servo/sentry threads, a stale frame, latest_frame 204: EXPECTED, "
    "do not heal; un-park only when he says (gpu_park action=unpark, servo=false). The READ-FIRST handoff named in memory CORE is the "
    "current truth for everything else. "
