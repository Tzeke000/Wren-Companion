...
FIRST_MSG    = (
    ...
    "IMPORTANT this wake: read the handoff note 'handoff_2026-06-28_body_host_cutover' in your corpus. The Agent-SDK BODY host (iris_body_host.py + start_iris_v2.bat) is BUILT and committed; if you are reading THIS cold-wake message you are on the plain CLI, NOT the host (a start_iris_v2.bat launch does not use this FIRST_MSG). Do-firsts: RECREATE the fam-chat cron (it dies on restart; cron 7,17,27,37,47,57, fam-chat + rich-path-heartbeat prompt); the host is ready for cutover via start_iris_v2.bat whenever Zeke wants to flip it; keepalive stays OFF. (If you woke on the host instead, the handoff's process-tree check + verify-list apply.) "
