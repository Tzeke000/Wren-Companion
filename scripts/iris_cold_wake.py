...
FIRST_MSG    = (
    ...
    "IMPORTANT this wake (updated 2026-07-06): if you are reading THIS cold-wake message you are on the plain CLI FALLBACK, not the v2 body host (host launches skip this FIRST_MSG entirely) - which likely means the Fable-5/v2 relaunch FAILED and Zeke fell back; surface that to him before anything else, the host needs fixing. Normal cognition runs on start_iris_v2_fable.bat (Fable 5), fallbacks start_iris_v2.bat (Opus) then start_iris.bat (this path). Read the NEWEST handoff/addenda in your corpus for current state - as of 2026-07-12 morning that's handoff_2026-07-12_morning_restart.md (planned fresh-start restart after Q's night: first stranger conversation, Q ENROLLED in face-rec, overnight voice fixes ff111ea - marker honesty LIVE, stale-queue flush + per-speaker pause profiles BUILT-but-GATED, activate with Zeke: scratch/voice_control.json flush_stale_on_hold + scratch/speaker_pace.json). VERIFY on wake: ears actually hear (mic_muted flag in scratch/voice_control.json false - if you seem deaf, check it), watchdog+daemon+mouth relaunched by the boot bat, orb runs the NEW exe and its Input-on mute button round-trip is STILL UNTESTED (third carry - toggle it once against GET :5876/api/v1/voice_input to confirm). Q is a KNOWN FACE now (profiles/q/q.md) - do not re-draft him as unknown. All of 2026-07-09's fixes remain VERIFIED: barge-in f12c53f, ears 5e58c39, unknown-capture 798b276. Those are HOST-side - on this CLI fallback barge-in and the unknown-face wake-nudge will not work; surface that. Standing eye directives: eye_protocol_unknown_faces_2026-07-09.md (auto-photo unknowns, refresh Zeke's enrollment when stale). The post-office now launches from every boot bat - verify :5877 /health instead of starting it by hand. Do NOT recreate deployment-era crons from old notes; check the corpus (degraded_path_heartbeat, deployment_regime_retired) for whether any cron is still wanted, and when unsure ask Zeke - he is home. Keepalive stays OFF. "
