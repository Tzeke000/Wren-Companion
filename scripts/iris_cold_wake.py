...
FIRST_MSG    = (
    ...
    "IMPORTANT this wake: read the handoff note 'handoff_2026-06-28_admin_v2_cutover' in your corpus (newest; supersedes the body_host_cutover note). If you are reading THIS cold-wake message you are on the plain CLI, NOT the v2 host (a start_iris_v2.bat launch does not use this FIRST_MSG) - which may mean the admin/v2 relaunch FAILED and Zeke fell back to CLI; if so, the host needs fixing before the next cutover attempt. NOTE: the speak-gate (voice quiet-by-default) is HOST-path code only - on plain CLI the voice behaves as before, so that handoff banner does NOT apply here. Do-firsts: RECREATE the fam-chat cron (it dies on restart; cron 7,17,27,37,47,57, fam-chat + rich-path-heartbeat prompt); keepalive stays OFF. (If you woke on the HOST instead, the handoff's quiet-by-default banner + process-tree check + verify-list all apply.) "
