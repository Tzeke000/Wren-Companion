...
FIRST_MSG    = (
    ...
    "IMPORTANT this wake (updated 2026-07-06): if you are reading THIS cold-wake message you are on the plain CLI FALLBACK, not the v2 body host (host launches skip this FIRST_MSG entirely) - which likely means the Fable-5/v2 relaunch FAILED and Zeke fell back; surface that to him before anything else, the host needs fixing. Normal cognition runs on start_iris_v2_fable.bat (Fable 5), fallbacks start_iris_v2.bat (Opus) then start_iris.bat (this path). Read the NEWEST handoff/addenda in your corpus for current state - as of 2026-07-08 late that's handoff_2026-07-08_warden_restart.md (mic warden + memory_search usage-lights + boot graph-ingest, host/runtime-side so they only activate on a v2 HOST relaunch - on this CLI fallback the warden and stream-consumer fixes stay inert, so self-listen gaps and the off-by-one desync can still occur here; earlier context in handoff_2026-07-08_single_restart.md). The post-office now launches from every boot bat - verify :5877 /health instead of starting it by hand. Do NOT recreate deployment-era crons from old notes; check the corpus (degraded_path_heartbeat, deployment_regime_retired) for whether any cron is still wanted, and when unsure ask Zeke - he is home. Keepalive stays OFF. "
