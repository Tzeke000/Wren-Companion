...
USE_PATH_A   = True
FIRST_MSG    = (
    "[REGIME STAMP 2026-09-08 22:5x - outranks every dated claim below] THE FILE DECIDES VOICE: read state/voice_deliberately_off.json. "
    "As of this stamp it reads off:true - Zeke turned voice OFF 2026-09-07 16:29 to free VRAM for gaming and has not turned it back on; "
    "mouth :8769 + daemon :8770 DOWN is CORRECT, do not heal; voice_speak falls back to piper (fine for a rare deliberate line). If the file "
    "is absent or off:false, voice is ON and both ports should be up. Toggle only via scripts/body_switch.ps1 on his word. EYES: UN-PARKED "
    "2026-09-08 18:56 on his word (you can if you want to) - gpu_park action=status should say parked:false; if a fresh runtime comes up "
    "parked (stale .tmp/body_pause.flag), gpu_park action=unpark servo=false. Servo/sentry OFF unless he asks. OLLAMA is DOWN because ZEKE "
    "QUIT IT 09-08 (little brain parked anyway) - not a fault. THIS RESTART (09-08 ~22:5x) was Zeke-permitted to land the llm_pending_poller "
    "in iris_body_host.py (62736d2): on a HOST boot expect the line [host] llm-pending idle nudge: ON. Camera PRESENT. The READ-FIRST "
    "handoff named in memory CORE (handoff_2026-09-08_evening_eyes_on_poller_pending.md) is the current truth for everything else. "
