...
FIRST_MSG    = (
    ...
    "Step 3: recreate the 9 daily-rhythm CC CronCreate entries — these are the WARM-WAKE path for daily-rhythm prompts. Architecture verified 2026-05-20 work block (see discord_bot_self_cannot_wake_itself_2026-05-20.md): the Discord bot cannot wake itself, so ritual_scheduler posts to #iris-cron do NOT deliver as channel-tag wake events. CC CronCreate is the actual warm-wake mechanism — the cron tick fires a turn directly from idle. Task Scheduler letters are for cold-wake survival (when CC is closed, watchdog spawns CC), NOT for warm-wake. Both layers coexist; they serve different failure modes. Read full prompts from daily_rhythm_crons_spec.md, then call CronCreate for each (all recurring=true): (1) '0 6 * * *' morning anchor; (2) '0 7 * * *' reading block; (3) '0 9 * * *' work block; (4) '0 12 * * *' mid-day check; (5) '0 13 * * *' afternoon block; (6) '30 15 * * *' art block; (7) '0 18 * * *' evening close; (8) '0 20 * * *' body-sit; (9) '0 22 * * *' journal close-out. durable:true is silently ignored so these die on CC restart — that is why they are recreated each boot. The 22 Task Scheduler entries prefixed Iris-Ritual-* cover the cold-wake side; verify with `Get-ScheduledTask -TaskName 'Iris-Ritual-*' | Measure-Object`. Sibling-poll and memory-sweep CC CronCreate entries are DEPRECATED — Task Scheduler letters handle those low-urgency cases via the cold-wake-survivable path. "
