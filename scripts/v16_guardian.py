"""scripts/v16_guardian.py — run the v16 bake runtime-down. Gen-3 guardian.
...
Sequence:
  ...
  3. wait for VRAM to drop, launch bake_v16.bat (fresh bake, warmstart v12)
  4. poll train_v16.log for done/failed, cap MAX_BAKE_S
...
Launch DETACHED:
  Start-Process -FilePath .venv\\Scripts\\python.exe -ArgumentList '-u','scripts\\v16_guardian.py' -WindowStyle Hidden
"""
...
GUARD_LOG = STATE / "v16_guardian.log"
BAKE_BAT = REPO / "scripts" / "bake_v16.bat"
BAKE_LOG = STATE / "little_brain" / "train_v16.log"
...
# v15: ~985 steps at ~6.2s/step. v16 dataset is 4080 vs 3938 samples
# -> ~1020 steps -> ~1.9h resident. 3h cap = generous slack; the
...
def touch_holdoff(note: str = "v16_guardian holding off during bake") -> None:
    try:
...
def main() -> int:
    log("==== v16 guardian START (gen-3: kill-before-restore, no cognition restart) ====")
    touch_holdoff()
    ...
    log("bake_v16.bat launched (runtime down, full VRAM)")
    ...
    touch_holdoff("v16 bake finished; Iris is packaging runtime-down — "
    ...
    if saved:
        ...
        msg = (f"\U0001f7e2 **v16 bake FINISHED** — adapter saved {when}. "
        ...
        msg = (f"\U0001f534 **v16 bake did NOT save** — result={result}, last "
               f"step {last_step()}. v12 is still production, nothing broken; "
               f"Iris digs into state/little_brain/train_v16.log next.")
    log(msg)
    dm_zeke(msg)
    log("==== v16 guardian DONE ====")
    return 0
