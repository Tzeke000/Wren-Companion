"""scripts/v17_guardian.py — run the v17 bake runtime-down. Gen-3 guardian.
...
Sequence:
  ...
  3. wait for VRAM to drop, launch bake_v17.bat (fresh bake, warmstart v12)
  4. poll train_v17.log for done/failed, cap MAX_BAKE_S
...
Launch DETACHED:
  Start-Process -FilePath .venv\\Scripts\\python.exe -ArgumentList '-u','scripts\\v17_guardian.py' -WindowStyle Hidden
"""
...
GUARD_LOG = STATE / "v17_guardian.log"
BAKE_BAT = REPO / "scripts" / "bake_v17.bat"
BAKE_LOG = STATE / "little_brain" / "train_v17.log"
...
# v15: ~985 steps at ~6.2s/step. v17 dataset is 4080 vs 3938 samples
...
def touch_holdoff(note: str = "v17_guardian holding off during bake") -> None:
    try:
...
def main() -> int:
    log("==== v17 guardian START (gen-3: kill-before-restore, no cognition restart) ====")
    touch_holdoff()
    ...
    log("bake_v17.bat launched (runtime down, full VRAM)")
    ...
    touch_holdoff("v17 bake finished; Iris is packaging runtime-down — "
    ...
    if saved:
        ...
        msg = (f"\U0001f7e2 **v17 bake FINISHED** — adapter saved {when}. "
        ...
        msg = (f"\U0001f534 **v17 bake did NOT save** — result={result}, last "
               f"step {last_step()}. v12 is still production, nothing broken; "
               f"Iris digs into state/little_brain/train_v17.log next.")
    log(msg)
    dm_zeke(msg)
    log("==== v17 guardian DONE ====")
    return 0
