"""scripts/v18_guardian.py — run the v18 bake runtime-down. Gen-3 guardian.
...
Sequence:
  ...
  3. wait for VRAM to drop, launch bake_v18.bat (fresh bake, warmstart v12)
  4. poll train_v18.log for done/failed, cap MAX_BAKE_S
...
Launch DETACHED:
  Start-Process -FilePath .venv\\Scripts\\python.exe -ArgumentList '-u','scripts\\v18_guardian.py' -WindowStyle Hidden
"""
...
GUARD_LOG = STATE / "v18_guardian.log"
BAKE_BAT = REPO / "scripts" / "bake_v18.bat"
BAKE_LOG = STATE / "little_brain" / "train_v18.log"
...
# v17: ~1020 steps at ~6.2s/step took ~2.1h. v18 dataset is 4497 vs 4370
# samples -> ~1125 steps -> ~2.0-2.2h resident. 3h cap = slack; the
...
def touch_holdoff(note: str = "v18_guardian holding off during bake") -> None:
    try:
...
def dm_zeke(text: str) -> None:
    try:
        ...
        h = {"Authorization": f"Bot {token}",
             ...
             "User-Agent": "IrisV16Guardian/1.0"}
...
def main() -> int:
    log("==== v18 guardian START (gen-3: kill-before-restore, no cognition restart) ====")
    touch_holdoff()
    ...
    log("bake_v18.bat launched (runtime down, full VRAM)")
    ...
    touch_holdoff("v18 bake finished; Iris is packaging runtime-down — "
    ...
    if saved:
        ...
        msg = (f"\U0001f7e2 **v18 bake FINISHED** — adapter saved {when}. "
               f"Runtime stays down while Iris packages to GGUF and runs the "
               f"5+5 battery (needs only ollama + :8772), then she restarts "
               f"cognition last. Production stays v16 — no flip without your go.")
    else:
        msg = (f"\U0001f534 **v18 bake did NOT save** — result={result}, last "
               f"step {last_step()}. v16 is still production, nothing broken; "
               f"Iris digs into state/little_brain/train_v18.log next.")
    log(msg)
    dm_zeke(msg)
    log("==== v18 guardian DONE ====")
    return 0
