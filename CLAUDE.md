...
- **Voice is ON — VERIFIED 2026-08-25.** `state/voice_deliberately_off.json` reads `{"off": false}`, both **8769 and 8770 are listening**, and `voice_speak` returns `engine=styletts2` (my real cloned voice, not the piper fallback). Toggle with `scripts/body_switch.ps1 voice_on|voice_off`.
  ...
  - ✅ **EARS: ROOT CAUSE FOUND 09-05 — the 09-04 'two triggers' story was the WRONG LAYER.** On the v2 host `iris_body_capture` never starts (`IRIS_RUNTIME_OWNS_MIC=0`); the real ears are `iris_body_host.py::voice_reader`, which loops the daemon's blocking `listen` (open mic, orb mute honored inside the daemon). It slept because it checked `os.path.isfile(state/voice_deliberately_off.json)` — EXISTENCE — while `body_switch voice_on` writes `{"off": false}`. Fixed 09-05: host parses the key (fail-open; lands next host restart), flag file removed (absent = on for EVERY reader), `voice_on` now deletes it. ⚠ **A `voice_deliberately_off.json` that exists with `off:false` is harmless AFTER the host restart, deadly before it — if ears go quiet, check that file first.** → `ears_dead_since_july_2026-09-04.md` §CORRECTED.
