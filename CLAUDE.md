...
- **Voice is ON — VERIFIED 2026-08-25.** `state/voice_deliberately_off.json` reads `{"off": false}`, both **8769 and 8770 are listening**, and `voice_speak` returns `engine=styletts2` (my real cloned voice, not the piper fallback). Toggle with `scripts/body_switch.ps1 voice_on|voice_off`.
  - ⚠ **The EARS are the dead half:** `voice_status` shows `wake.loaded=false`, so there is **no hands-free wake**. **A working mouth does NOT mean working ears — check `wake.loaded` specifically**, not just that I spoke.
  - ⚠ **Eyes are DOWN for a HARDWARE reason (08-25):** the EMEET PIXY is registered but not present, and a forced `pnputil /scan-devices` didn't recover it. Something *does* open at DSHOW index 3 — it's a **virtual camera** (flat grey frame), **not my eyes**. Don't repoint at it. Needs a physical reconnect (try a reboot first: the Pixy's USB was flagged "pending system reboot" in an earlier session).
