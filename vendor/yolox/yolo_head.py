...
# VENDORED EDIT 2026-08-25: repointed from `yolox.utils` to a local shim so
# this runs without installing the YOLOX package (which would pull torchvision
# + a setup.py build). Only `meshgrid` is used on the inference path; the other
# three are training-only and the shim raises if they are ever reached.
from ._utils_shim import bboxes_iou, cxcywh2xyxy, meshgrid, visualize_assign
