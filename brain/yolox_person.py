...
def bridge(frame, *, last_face_seen_ts: float,
           min_score: float = _MIN_SCORE) -> dict | None:
    """Carry an ALREADY-ESTABLISHED identity across a short face occlusion.

    Returns a body record, or None. Refuses — deliberately and often — because
    every refusal is the module declining to guess who someone is:
      ...
      * more than one person  -> no way to know which body is the one the face
        pipeline identified. Two candidates, no evidence, no answer.
