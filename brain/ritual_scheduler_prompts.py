...
PROMPTS: dict[str, dict[str, str]] = {
    ...
    "memory_sweep": {
        "schedule": "17 */4 * * *",
        "prompt": (
            "Memory sweep + schedule audit. Two parts:\n\n"
            "PART 1 — schedule audit (per Zeke 2026-05-21): check whether "
            "the daily-rhythm blocks since the last sweep fired on time. "
            "Run `Get-ScheduledTask -TaskName 'Iris-Ritual-*' | "
            "Get-ScheduledTaskInfo | Where-Object { $_.LastRunTime -gt "
            "(Get-Date).AddHours(-4) } | Select-Object TaskName, LastRunTime, "
            "LastTaskResult`. For each fire, compute drift = "
            "LastRunTime - scheduled time. **If any fire is more than ~5 min "
            "off schedule, that becomes the PRIMARY fix this sweep — "
            "investigate why (Task Scheduler health, watcher liveness, "
            "path-E truncation, CC unresponsive, system clock skew, etc.) "
            "and either fix or surface to Zeke explicitly.** Schedule "
            "reliability is load-bearing for deployment — drift compounds "
            "and the whole inhabit-mode regime depends on blocks firing "
            "when they should. ~5 min slop is fine; more is a fix-priority. "
            "Also check `.tmp/cron_inject_watcher_heartbeat.txt` freshness "
            "(<5s mtime) — stale watcher = blocks won't deliver even if "
            "Task Scheduler fires.\n\n"
            "PART 2 — review last 4 hours of conversation context for: "
            "(a) any structural rules, principles, or patterns NAMED but "
            "not yet written to a memory file; (b) any surprising or "
            ...
            "type) and update MEMORY.md index with a one-line pointer.\n\n"
            "If schedule is clean AND nothing new is worth filing, output "
            "one short line confirming the sweep ran with no drift + "
            "nothing to file, then stop."
        ),
    },
    "sibling_poll_waking": {
