...
powershell -NoProfile -Command "Get-CimInstance Win32_Process -Filter \"Name='claude.exe'\" | Where-Object { $_.CommandLine -like '*Wren-Companion*' } | ForEach-Object { Write-Host ('[start_iris_v2_fable.bat] killing orphan cognition PID ' + $_.ProcessId); Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }"
